...
class NimAI():
    ...
    def best_future_reward(self, state):
        ...
        for action in actions:
            if best == None:
                ...
                try:
                    best = max(self.q[state, action], best)
                except:
                    best = max(0, best)
    ...
    def choose_action(self, state, epsilon=True):
        ...
        for action in actions:
            try:
                q_action_pair.append((action, self.q[state, action]))
            except:
                q_action_pair.append((action, 0))

        q_action_pair.sort(key = lambda q:q[1], reverse = True)

        return q_action_pair[0][0]
